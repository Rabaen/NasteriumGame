...
public class RayScript: MonoBehaviour {
	...
	float hoverHeightStrictness = 5.0f;

	public float bankAmount = 3.7f; //0.1
	public float bankSpeed = 0.2f;  //0.2
	Vector3 bankAxis = new Vector3(0.0f, 0.0f, -1.0f); //-1,0,0
	public float turnSpeed = 20.0f; //20
	...
	void  FixedUpdate (){
		...
		transform.position = Vector3.Lerp(transform.position, new Vector3(transform.position.x, Cube.transform.position.y + hoverHeight, transform.position.z), hoverHeightStrictness); //lerp back to hover hieght
